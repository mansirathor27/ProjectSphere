...
const DeadlinesPage = () => {
  ...
  const handleSubmit = async(e)=>{
    ...
    let deadlineData = {name: selectedProject?.student?.name,
    ...
    try {
      const updated = await dispatch(
        createDeadline({ id: selectedProject._id, data: deadlineData})).unwrap();
      const updatedProject = updated?.project || updated;

      if(updatedProject?._id){
        setViewProjects((prev)=>prev.map((p)=> p._id === updatedProject._id ? {
          ...p, ...updatedProject} : p
        ));
      }
  ...
  <div className="space-y-6">
    {/* header */}
    <div className="card">
      <div className="card-header flex flex-col md:flex-row justify-between items-start
      md:items-center">
        <div>
          <h1 className="card-title">Manage Deadlines</h1>
          <p className="card-subtitle">Create and monitor project deadlines</p>
        </div>
        <button onClick={()=> setShowModal(true)} className="btn-primary mt-4 md:mt-8">
    ...
    <div className="card">
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1">
          ...
          <input type="text" placeholder="Search by project or student..." className="input-field w-full" value={searchTerm}
        ...
        <table className="w-full">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase
              tracking-wider">Project Title</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase
              tracking-wider">Supervisor</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase
              tracking-wider">Deadline</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase
              tracking-wider">Updated</th>
            </tr>
          </thead>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4 max-h-screen overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-semibold text-slate-900">Create or Update Deadlines</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" className="input" placeholder="Start typing or search projects..."
                    ...
                    query && !selectedProject && (
                      <div className="mt-2 border border-slate-200 rounded-md max-h-56 overflow-y-auto">
                        {(projects || []).filter((p)=>
                          ...
                          <button type="button" key={p._id} className="w-full text-left px-3 py-2 hover:bg-slate-50" onClick={()=>{
                  ...
                  <div>
                    ...
                    <input type="date" className="input-field w-full" disabled={!selectedProject}
