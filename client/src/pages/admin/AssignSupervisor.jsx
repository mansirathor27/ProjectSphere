    ...
    <div className="space-y-6">
      <div className="card">
        <div className="card-header">
          ...
          <p className="car-subtitle">Manage supervisor assignments for students and projects</p>
        </div>
      </div>
...
{/* filter */}
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1">
          ...
          <input type="text" placeholder="Search by student name or project title..." className="input-field w-full" value={searchTerm}
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {
                filtered.map((row)=>(
                  <tr key={row.projectId} className="hover:bg-slate-50">
                    ...
                    <td className="px- py-4">{row.title}</td>
                    ...
                    <td className="px- py-4">{row.deadline}</td>
                    <td className="px- py-4">{row.updatedAt}</td>
                    <td className="px- py-4 whitespace-nowrap">
                      ...
                      onChange={(e)=> handleSupervisorSelect(row.projectId, e.target.value)}>
                        <option value="" disabled>Select Supervisor</option>
                        {
                          ...
                          .map((t)=>(
                            <option value={t._id} key={t._id}>{t.name} ({t.capacityLeft} slots left)</option>
                          ))
                        }
                      </select>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button className="btn-primary text-sm w-40" onClick={()=> handleAssign(row.studentId, row.status, row.projectId)}
                        disabled={pendingFor === row.projectId || !!row.supervisor || row.status === "rejected"
                          || !row.isApproved || !selectedSupervisor[row.projectId]
                        }>
                        {
                          pendingFor === row.projectId ? "Assigning..." : row.supervisor ?
                          "Assigned" : row.status === "rejected" ? "Rejected" : !row.isApproved ? 
                          "Not Approved" : "Assign"
                        }
                      </button>
                    </td>
