...
const SupervisorPage = () => {
  ...
  <div className="space-y-6">
                    ...
                    <div>
                      <label className="text-sm font-medium text-slate-500 uppercase tracking-wide">
                        ...
                        <p className="text-slate-800 font-medium">
                          {Array.isArray(supervisor?.experties) ? supervisor.experties.join(", ") : 
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-lg text-slate-800 font-semibold mt-1">
                    {project?.deadline ? formatDeadline(project.deadline) :"No deadline set"}</p>
                </div>
                 <div>
                  <label className="text-sm font-medium text-slate-500 uppercase tracking-wide">
                  Created</label>
                  <p className="text-lg text-slate-800 font-semibold mt-1">
                    {project.createdAt ? formatDeadline(project.createdAt) :"Unknown"}</p>
                </div>
              </div>
            </div>
    ...
    {/* Available supervisors | Only when project exists and no supervisor is assigned */}
  ...
  <div className="card">
    <div className="mb-4">
      ...
      <p className="text-sm text-slate-600">
        Choose a supervisor and send a request
      </p>
    </div>
          ...
          <div className="modal-content">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg fon-medium text-slate-800">Request Supervision</h3>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-slate-40 rounded-md">
                  <p className="text-sm text-slate-600">{selectedSupervisor?.name}</p>
                </div>

                <div>
                  <label className="label">Message to supervisor</label>
                  <textarea className="input min-h-[120px]" required value={requestMessage} onChange={(e)=>{
                    setRequestMessage(e.target.value) }}placeholder="Introduce yourself and explain why you'd write this professor to supervise your project"
                />
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t border-slate-200">
                  ...
                  <button onClick={submitRequest} className="btn-primary" disabled={!requestMessage.trim()}>
                    Send Request
                  </button>
                </div>
              </div>
