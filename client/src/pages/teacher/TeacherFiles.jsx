  ...
  <div className="space-y-6">
    ...
    {/* controller */}
      ...
      <div className="flex items-center gap-4">
        ...
        <input type="text" className="input w-96" placeholder="Search files..." 
          ...
          {fileStats.map((item, i)=>{
            return(
              <div key={i} className={`${item.bg} p-4 rounded-lg`}>
                ...
                <p className={`text-2xl ${item.value} font-bold`}>{item.count}</p>
              </div>
            )
          })}
        </div>
            ...
            filteredFiles.map((file)=>(
              <div key={file.id} className="card">
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="font-medium text-slate-800 mb-1 truncate w-full" title={file.name}>
                    {file.name}
                  </h3>
                  ...
                  <div className="flex gap-2 w-full">
                    <button onClick={()=>handleDownloadFile(file)} className="rounded-lg text-white 
                    bg-blue-600 text-lg font-medium flex items-center justify-center w-full py-3 gap-3 hover:bg-blue-700 
                ...
                filteredFiles.map((file)=>{
                  return (
                    <tr key={file.id} className="border-t hover:bg-slate-50 transition-colors">
                      <td className="py-3 px-4 text-center gap-3">
