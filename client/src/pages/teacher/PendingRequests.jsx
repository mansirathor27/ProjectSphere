...
const PendingRequests = () => {
  ...
  const getProjectTitleFromRequest = (request) => {
    return (
...
dispatch(getTeacherRequests(authUser._id)); // ✅ refresh list
dispatch(getAssignedStudents()); // ✅ update assigned students
dispatch(getTeacherDashboardStats());
  ...
  const filteredRequests = list.filter((request) =>{
      const requestProjectTitle =
        request?.student?.project?.title ??
        getProjectTitleFromRequest(request);
      const matchesSearch = 
      (request?.student?.name || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
      (requestProjectTitle || "").toLowerCase().includes(searchTerm.toLowerCase());
  ...
  <div className="space-y-6">
    ...
    <div className="card">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
          <input type="text" placeholder="Search by student name or project title.." className="input-field flex-1"
    ...
    <div className="space-y-4">
      {
        filteredRequests.map((req)=>{
          const id = req._id;
          // In your current DB, the project title/status appears under `req.student.project`
          // (not under `req.project`), so we fallback accordingly.
          const project = req.project || req.student?.project;
          const projectTitle =
            project?.title ??
            ...
            <div className="flex flex-col lg:flex-row justify-between">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={`badge ${req.status === "pending" ? "badge-pending" : req.status
                    === "accepted" ? "badge-approved" : "badge-rejected"
                  }`}>
              ...
              {/* actions */}
                {
                  req.status === "pending" && (
                    <div className="flex items-center gap-3 mt-3">
                      <button className={`px-4 py-1.5 text-sm rounded-lg font-medium transition-colors duration-200 
                      disabled:cursor-not-allowed disabled:opacity-60
                        ${canAccept ? "bg-green-600 hover:bg-green-700 text-white"
                      ...
                      <button className={`px-4 py-1.5 text-sm rounded-lg font-medium transition-colors 
                      duration-200 disabled:opacity-60 bg-red-600 hover:bg-red-700 text-white
